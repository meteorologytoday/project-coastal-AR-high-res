...
data_ave  = dlh.loadDataByDate(test_dt, msm, **crop_kwargs, datasets=["diag_Sbdgt", "diag_2D", "diag_state",])
...
MLG["dMLSdt"] = (
      mlt.computeMLMean(data_snp2["SALT"], MLD2_threshold, z_W, mask=mask, fill_value=np.nan)  
    - mlt.computeMLMean(data_snp1["SALT"], MLD1_threshold, z_W, mask=mask, fill_value=np.nan)  
) / 86400.0



MLG["G_ent2"] = MLG["dMLSdt"] - ( MLG["G_adv"] + MLG["G_vdiff"] + MLG["G_sfc"] + MLG["G_fwf"] )
MLG["G_ent"] = (
...
MLG["G_ttl_surf"] = ds["TOTSTEND"].to_numpy()[1:2, :, :].mean(axis=0)
...
plot_varnames = ["dMLSdt", "G_adv", "G_vdiff", "G_ent", "G_sfc", "G_fwf", "G_ent2"]
...
for i, varname in enumerate(plot_varnames):
    ...
    cb.ax.set_ylabel(" %s [ $ \\mathrm{PSU} / \\mathrm{s} $ ]" % (varname,))
